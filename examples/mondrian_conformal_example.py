...
def main():
    ...
    print(f"  Coverage:        {marg['coverage']['rate']:.1%} "
          f"(CI: [{marg['coverage']['ci_95'][0]:.3f}, {marg['coverage']['ci_95'][1]:.3f}])")
    print(f"  Singleton rate:  {marg['singletons']['rate']:.1%} "
          f"(automated decisions)")
    print(f"  Escalation rate: {marg['abstentions']['rate'] + marg['doublets']['rate']:.1%} "
          f"(requires human review)")
    
    sing_err = marg['singletons']['errors']
    print(f"\nSingleton Error Rate:")
    print(f"  Overall: {sing_err['rate']:.1%} "
          f"(CI: [{sing_err['ci_95'][0]:.3f}, {sing_err['ci_95'][1]:.3f}])")
    ...
    if marg['pac_bounds']['rho'] is not None:
        ...
        if pac['alpha_singlet_observed'] <= pac['alpha_singlet_bound']:
            print(f"  ✓ PAC guarantee satisfied!")
        else:
