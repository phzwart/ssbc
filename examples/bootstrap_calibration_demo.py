...
from ssbc import BinaryClassifierSimulator, ssbc_correct, split_by_class
...
def _bootstrap_single_trial(
    ...
    try:
        ssbc_0 = ssbc_correct(
            ...
            n=class_data_boot[0]['n'],
            delta=delta
        )
        ssbc_1 = ssbc_correct(
            alpha_target=alpha_target,
            n=class_data_boot[1]['n'],
            delta=delta
        )
    except Exception:
...
def bootstrap_calibration_uncertainty(
    ...
    results = Parallel(n_jobs=n_jobs)(
        delayed(_bootstrap_single_trial)(
            labels, probs, alpha_target, delta, test_size, bs_seed, simulator
        )
    ...
    return {
        ...
        'test_size': test_size,
        'marginal': {
            metric: compute_stats([r[metric] for r in results])
            for metric in metrics
        },
        'class_0': {
            metric: compute_stats([r[f'{metric}_0'] for r in results])
            for metric in metrics
        },
        'class_1': {
            metric: compute_stats([r[f'{metric}_1'] for r in results])
            for metric in metrics
        },
    }
